...
- **Progress activity tracking**: `lesson_progress.updated_at` is used as the authoritative timestamp for learner activity so "active learners" can mean any lesson progress during the selected period, not only completions.
...
- **Authorization**: Instructors may only query courses they own (`courses.instructorId = currentUserId`). Admins may query any course and filter by any instructor. Authorization is enforced inside the analytics service using the viewer identity, not only at the route layer.
...
Create the `analyticsService` aggregation module as the single source of truth for all metric definitions. Expose a `getRollupMetrics(instructorId, days, viewerId, viewerRole)` function that returns aggregate KPIs across all courses owned by the instructor: gross revenue, new enrollments, active learners, and completion rate for the selected time range.
...
Extend `analyticsService` with a `getCourseRows(instructorId, days, viewerId, viewerRole)` function that returns one row per owned course containing: course title, revenue, new enrollments, active learners, completion rate, average rating + count, and a set of boolean warning flags.
