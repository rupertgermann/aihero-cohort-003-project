...
import { CourseStatus, LessonCommentStatus, UserRole } from "~/db/schema";
...
const courseEditorActionSchema = v.variant("intent", [
  v.object({
    ...
    title: v.pipe(v.string(), v.trim(), v.minLength(1, "Title cannot be empty.")),
  }),
  v.object({
    intent: v.literal("update-description"),
    description: v.pipe(v.string(), v.trim(), v.minLength(1, "Description cannot be empty.")),
  }),
  v.object({
  ...
  v.object({
    ...
    title: v.pipe(v.string(), v.trim(), v.minLength(1, "Module title cannot be empty.")),
  }),
  v.object({
  ...
  v.object({
    ...
    moduleId: coerceInt,
    title: v.pipe(v.string(), v.trim(), v.minLength(1, "Lesson title cannot be empty.")),
  }),
  v.object({
  ...
  v.object({
    intent: v.literal("hide-comment"),
    commentId: coerceInt,
  }),
  v.object({
    intent: v.literal("restore-comment"),
    commentId: coerceInt,
  }),
]);
...
export async function loader({ params, request }: Route.LoaderArgs) {
  ...
  const comments = getLessonCommentsForCourse(courseId);
...
export async function action({ params, request }: Route.ActionArgs) {
  ...
  if (intent === "hide-comment" || intent === "restore-comment") {
    ...
    if (!mod || mod.courseId !== courseId) {
      return data(
        { error: "Comment not found in this course." },
        { status: 404 }
      );
    }
...
function CommentModerationAction({
  commentId,
  status,
}: {
  commentId: number;
...
function CourseCommentsTab({
  ...
  const visibleCount = comments.filter(
    (comment) => comment.status === LessonCommentStatus.Visible
  ).length;
    ...
    <div className="mx-auto max-w-7xl p-6 lg:p-8">
      ...
      <Tabs defaultValue="content">
        ...
        <TabsContent value="comments" className="mt-6">
          <CourseCommentsTab comments={comments} courseSlug={course.slug} />
        </TabsContent>
      </Tabs>
    </div>
  );
